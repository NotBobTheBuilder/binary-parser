...
var Parser = /** @class */ (function () {
    ...
    Parser.prototype.prefixed = function (varName, options) {
        if (!options.prefix) {
            ...
            throw new Error("Specified primitive type \"" + options.prefix + "\" is not supported.");
        }
    ...
    Parser.prototype.prefixedArray = function (varName, options) {
        if (!options.prefix) {
    ...
    Parser.prototype.getCompiledContext = function () {
    ...
    Parser.prototype.getCompiledContextEncode = function () {
        var ctx = new context_1.Context();
        if (!this.alias) {
    ...
    Parser.prototype.compile = function () {
        var compiledContext = this.getCompiledContext();
        var src = '(function(buffer, constructorFn) { ' + compiledContext.code + ' })';
        this.compiled = vm_1.runInNewContext(src, Object.assign({}, compiledContext.functionReferences, { Buffer: buffer_1.Buffer }));
    };
    Parser.prototype.compileEncode = function () {
        var compiledContext = this.getCompiledContextEncode();
        var src = '(function(obj) { ' + compiledContext.code + ' })';
        this.compiledEncode = vm_1.runInNewContext(src, Object.assign({}, compiledContext.functionReferences, { Buffer: buffer_1.Buffer, SmartBuffer: smart_buffer_1.SmartBuffer }));
    };
    ...
    Parser.prototype.generate_encodeString = function (ctx) {
        ...
        if (this.options.length) {
            ...
            if (this.options.stripNull) {
                ctx.pushCode("if (" + padLen + " > 0) {smartBuffer.writeString('\0'.repeat(" + padLen + "));}");
            }
    ...
    Parser.prototype.generatePrefixed = function (ctx) {
        var prefix = this.options.prefix;
        var includePrefixInLength = this.options.includePrefixInLength;
        var type = this.options.type;
        var lengthInBytes = ctx.generateTmpVariable();
        var savBuffer = ctx.generateTmpVariable();
        var savOffset = ctx.generateTmpVariable();
        var item = ctx.generateVariable(this.varName);
        if (aliasRegistry[prefix]) {
            var tempVar = ctx.generateTmpVariable();
            ctx.pushCode("var " + tempVar + " = " + (FUNCTION_PREFIX + prefix) + "(offset);");
            ctx.pushCode("var " + lengthInBytes + " = " + tempVar + ".result; offset = " + tempVar + ".offset;");
            if (prefix !== this.alias)
                ctx.addReference(prefix);
        }
        else {
            var typeName = CAPITILIZED_TYPE_NAMES[prefix];
            ctx.pushCode("var " + lengthInBytes + " = buffer.read" + typeName + "(offset);");
            ctx.pushCode("offset += " + PRIMITIVE_SIZES[prefix] + ";");
        }
        if (includePrefixInLength) {
            ctx.pushCode(lengthInBytes + " -= " + PRIMITIVE_SIZES[prefix]);
        }
    ...
    Parser.prototype.generate_encodePrefixed = function (ctx) {
        var prefix = this.options.prefix;
        var includePrefixInLength = this.options.includePrefixInLength;
        var type = this.options.type;
        var lengthInBytes = ctx.generateTmpVariable();
        ...
        if (type instanceof Parser) {
            ...
            ctx.pushCode("smartBuffer.write" + CAPITILIZED_TYPE_NAMES[type] + "(" + name + ");");
        }
        else {
            ctx.pushCode("smartBuffer.writeBuffer(" + (FUNCTION_ENCODE_PREFIX + type) + "(" + name + "));");
            if (type !== this.alias) {
                ctx.addReference(type);
            }
        }
        ...
        // Copy tmp Buffer to saved smartBuffer
        ctx.pushCode(savSmartBuffer + ".write" + CAPITILIZED_TYPE_NAMES[prefix] + "(" + lengthInBytes + ")");
        ctx.pushCode(savSmartBuffer + ".writeBuffer(" + tmpBuffer + ");");
        // Restore current smartBuffer
        ctx.pushCode("smartBuffer = " + savSmartBuffer + ";");
    };
    Parser.prototype.generatePrefixedArray = function (ctx) {
        var prefix = this.options.prefix;
        var includePrefixInLength = this.options.includePrefixInLength;
        var type = this.options.type;
        var lengthInBytes = ctx.generateTmpVariable();
        var array = ctx.generateVariable(this.varName);
        var counter = ctx.generateTmpVariable();
        var item = ctx.generateTmpVariable();
        if (aliasRegistry[prefix]) {
            var tempVar = ctx.generateTmpVariable();
    ...
    Parser.prototype.generate_encodePrefixedArray = function (ctx) {
        var prefix = this.options.prefix;
        var includePrefixInLength = this.options.includePrefixInLength;
        var type = this.options.type;
        var lengthInBytes = ctx.generateTmpVariable();
